...
import { getChildEnrolledMentors, getParentProfile, startConversationWithMentorAsParent, getOrCreateSupportChatRoom, getMessages, sendMessage } from "@/app/actions";
import { validateMessage, sanitizeText } from "@/lib/validate";

type MentorContact = Awaited<ReturnType<typeof getChildEnrolledMentors>>[number];
...
export default function MessagesPage() {
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- deliberate fetch-driven-by-childId-param
    setLoading(true);
    Promise.all([getParentProfile(), childId ? getChildEnrolledMentors(childId) : Promise.resolve([])])
  ...
  useEffect(() => {
    if (activeId !== SUPPORT_ID && !childId) return;
    ...
    setRoomLoading(true);
    setRoomError(null);
    const resolveRoom = activeId === SUPPORT_ID ? getOrCreateSupportChatRoom() : startConversationWithMentorAsParent(activeId, childId!);
    resolveRoom
      .then((roomId) => {
        if (cancelled) return;
  ...
  }, [activeId, roomIdByMentor, childId]);
  ...
  return (
    <div className="-m-4 h-[calc(100%+2rem)] sm:m-0 sm:h-auto sm:space-y-4">
      <div className="hidden sm:block">
        ...
        <p className="text-[13px] text-[#4A5A7A] mt-0.5">Chat with Gadha Online Support or your enrolled mentors.</p>
      </div>
