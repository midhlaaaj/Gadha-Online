...
type Notification = Awaited<ReturnType<typeof getAdminNotifications>>[number];
...
function NotifIcon({ type }: { type: string }) {
  if (type === "new_booking_request")
    return (
      <span className="w-8 h-8 rounded-full bg-[#E6F1FB] flex items-center justify-center shrink-0">
        <IconCalendarPlus className="w-4 h-4 text-[#2F7FE8]" />
      </span>
    );
  if (type === "mentor_invite_accepted")
    return (
  ...
  return (
    <span className="w-8 h-8 rounded-full bg-amber-50 flex items-center justify-center shrink-0">
      <IconMessageCircle className="w-4 h-4 text-amber-500" />
    </span>
  );
}
...
export function AdminNotificationBell() {
  ...
  useEffect(() => {
    getUnreadAdminNotificationCount().then(setUnreadCount).catch(() => {});
  }, []);
        ...
        <div className="absolute right-0 top-11 w-80 bg-white rounded-2xl border border-[#D0DCF5] shadow-2xl shadow-[#1B3A6B]/10 z-50 overflow-hidden">
            ...
            {loading ? (
              <div className="space-y-3 p-4">
                {[...Array(3)].map((_, i) => (
                  <div key={i} className="flex gap-2">
                    <div className="w-8 h-8 rounded-full bg-slate-100 animate-shimmer shrink-0" />
                    <div className="flex-1 space-y-1.5 pt-1">
                      <div className="h-2.5 w-36 rounded bg-slate-100 animate-shimmer" />
                      <div className="h-2 w-52 rounded bg-slate-100 animate-shimmer" />
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="divide-y divide-[#F0F3FB]">
                {notifications.map((n) => (
                  <Link
                    key={n.id}
                    href={n.link_url || "/admin/dashboard"}
          ...
          {notifications.length > 0 && (
            <div className="px-4 py-2.5 border-t border-[#F0F3FB]">
              <Link href="/admin/dashboard" onClick={() => setOpen(false)} className="text-[11px] font-bold text-[#2F7FE8] hover:underline">
                View dashboard →
              </Link>
            </div>
          )}
