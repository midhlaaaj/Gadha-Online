-- SQL script to accommodate course bookings in the database (run this in your Supabase SQL Editor)
...
-- 4. Re-create the unique constraint for session bookings (allowing NULLs for course bookings)
ALTER TABLE public.bookings ADD CONSTRAINT unique_student_session_booking UNIQUE (student_id, session_id);

-- 5. Create a new unique constraint for course bookings (allowing NULLs for session bookings)
